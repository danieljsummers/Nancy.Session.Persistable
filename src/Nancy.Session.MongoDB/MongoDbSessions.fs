...
type MongoSession() =
  ...
  member this.ToSession () : IPersistableSession =
    upcast BasePersistableSession(Guid.Parse this.Id, this.LastAccessed,
                                  JsonConvert.DeserializeObject<Dictionary<string, obj>> this.Data)
...
and MongoDBSessionStore (cfg : MongoDBSessionConfiguration) =
  ...
  let firstOrDefault (cursor : IAsyncCursor<MongoSession>) = cursor.FirstOrDefaultAsync()
                                                             |> await
  ...
  interface IPersistableSessionStore with
    ...
    member this.UpdateLastAccessed id =
      ...
      collection().UpdateOneAsync((byId <| string id),
                                  Builders<MongoSession>.Update.Set((fun sess -> sess.LastAccessed), DateTime.Now))
      |> await
      |> ignore

    member this.UpdateSession session =
      dbg (fun () -> sprintf "Updating session data for session Id %s" (string session.Id))
      collection().ReplaceOneAsync((byId <| string session.Id),
                                   MongoSession(
                                     Id           = string session.Id,
                                     LastAccessed = (match cfg.UseRollingSessions with
                                                     | true -> DateTime.Now
                                                     | _ -> session.LastAccessed),
    ...
    member this.ExpireSessions () =
      ...
      collection()
        .DeleteManyAsync(Builders<MongoSession>.Filter.Lt((fun sess -> sess.LastAccessed), DateTime.Now - cfg.Expiry))
      |> await
      |> ignore
